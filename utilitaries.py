...
def sendMail(error, text="", lvl=-1, str_tweet="", tweet=None):
    CONST = _Const()
    body = ""
    ...
    if not error and tweet is not None and str_tweet != "":
        msg['Subject'] = 'Le ' + datetime.now().strftime(
            "%d/%m/%Y") + ' : On a passer le seuil de ' + str(lvl)  # Object pour dire que tout vas bien
        # corp du message
        body = "Coucou patronne,\n\n" + \
               "On a passer un seuil de 10 000. Tu peut aller prendre un screen capt sur le site de la pétition https://petitions.senat.fr/initiatives/i-416\n\n" + \
               "\n\nTu peut voir le tweet ici https://twitter.com/" + tweet.user.screen_name + "/status/" + str(tweet.id) + \
               "A demain\n\n" + \
               "Handi Bot\n\n"

    elif not error and tweet is not None and str_tweet == "": #SI il n'y a pas d'erreur et que l'app a tweeté ALORS
        msg['Subject'] = 'Le ' + datetime.now().strftime("%d/%m/%Y") + ' : On a passer un seuil de pourcentage' #Object pour dire que tout vas bien
        #corp du message
        body = "Coucou patronne,\n\n"+\
                         "On a passer un seuil de pourcentage, j'ai donc tweeté\n\n" + \
                        "\n\nJe te copie/colle le tweet en pièces jointes."+\
                        "\n\nTu peut voir le tweet ici https://twitter.com/" + tweet.user.screen_name + "/status/" + str(tweet.id) + \
                        "\n\nA demain"+\
                        "\n\nHandi Bot"
        #On ajoute les pièces jointes
    elif error == 0 and tweet is None and str_tweet == "":  # SI il n'y a pas d'erreur et que l'app a tweeté ALORS
        msg['Subject'] = 'Le ' + datetime.now().strftime(
            "%d/%m/%Y") + ' : La fin '  # Object pour dire que tout vas bien
        # corp du message
        body = "Coucou patronne,\n\n" + \
               "Dans 10 signatures c'est la fin \n\n" + \
               "\n\nA demain" + \
               "\n\nHandi Bot"
        # On ajoute les pièces jointes
        #add_attachments(msg)
